...
def EEGNet(nb_classes, Chans = 64, Samples = 128, 
             dropoutRate = 0.25, kernLength = 256, F1 = 8, 
             D = 2, F2 = 16, norm_rate = 0.25, dropoutType = 'Dropout', 
             trials=128, regularize = [False, 'L1', 0.01]):
    """ Keras Implementation of EEGNet
    kernLength was default 64, changed to 125 as 0.5 x 250Hz
    ...
    # Removed by GTL to remove first Conv2D
    if regularize[0] == False:  # No need regularizer
        block1       = Conv2D(F1, (1, kernLength), padding = 'same',
                                       input_shape = (1, Chans, Samples),
                                       use_bias = False,
                                       data_format='channels_first')(input1)   # By GTL channel_first
        print('block1 first stage shape = {}. F1 = {}, kernLength = {}, Chans = {}, Samples = {}'.format(block1.shape, F1, kernLength, Chans, Samples))
    elif regularize[1] == 'L1': # Need L1 regularizer
        block1       = Conv2D(F1, (1, kernLength), padding = 'same',
                                       input_shape = (1, Chans, Samples),
                                       use_bias = False,
                                       kernel_regularizer = tf.keras.regularizers.L1(regularize[2]),
                                       data_format='channels_first')(input1)   # By GTL channel_first
    elif regularize[1] == 'L2': # Need L2 regularizer
        block1       = Conv2D(F1, (1, kernLength), padding = 'same',
                                       input_shape = (1, Chans, Samples),
                                       use_bias = False,
...
#     ############################################################################################################
#     # Start: Added by GTL to test add layer ####################################################################
#     ############################################################################################################
#     block1       = Activation('elu')(block1)
    
# #     # Create trainable tf.Variable to multiply (default to -1) with noise electrode channels
# #     noise_weight = tf.Variable((np.ones((12*4*250),dtype=np.float32).reshape(-1,12,4,250)),name='noise_weight')*-1
# #     print('Noise weight shape is {}'.format(noise_weight.shape))
# #     print(block1[:,:,4:,:].shape)
        
# #     block1 = block1[:,:,4:,:] * noise_weight + block1[:,:,0:4,:]
...
#     ch0 = Concatenate(axis=2)([tf.expand_dims(block1[:,:,0,:], axis=2),tf.expand_dims(block1[:,:,4,:], axis=2)])
#     ch1 = Concatenate(axis=2)([tf.expand_dims(block1[:,:,1,:], axis=2),tf.expand_dims(block1[:,:,5,:], axis=2)])
#     ch2 = Concatenate(axis=2)([tf.expand_dims(block1[:,:,2,:], axis=2),tf.expand_dims(block1[:,:,6,:], axis=2)])
#     ch3 = Concatenate(axis=2)([tf.expand_dims(block1[:,:,3,:], axis=2),tf.expand_dims(block1[:,:,7,:], axis=2)])
    
#     print('ch0,ch1,ch2,ch3 before Conv2D shape = {}, {}, {}, {}'.format(ch0.shape, ch1.shape, ch2.shape, ch3.shape))
    
#     ch0        = Dense(250, name = 'dense1', use_bias=False,
#                              kernel_constraint = max_norm(norm_rate))(ch0)
#     ch1        = Dense(250, name = 'dense2', use_bias=False,
#                              kernel_constraint = max_norm(norm_rate))(ch1)
#     ch2        = Dense(250, name = 'dense3', use_bias=False,
#                              kernel_constraint = max_norm(norm_rate))(ch2)
#     ch3        = Dense(250, name = 'dense4', use_bias=False,
#                              kernel_constraint = max_norm(norm_rate))(ch3)
        
# #     ch0 = Conv2D(12, (1, kernLength), padding = 'same',
# #                                        input_shape = (1, Chans, Samples),
# #                                        use_bias = False,
# #                                        data_format='channels_first')(ch0)
# #     ch1 = Conv2D(12, (1, kernLength), padding = 'same',
# #                                        input_shape = (1, Chans, Samples),
# #                                        use_bias = False,
# #                                        data_format='channels_first')(ch1)
# #     ch2 = Conv2D(12, (1, kernLength), padding = 'same',
# #                                        input_shape = (1, Chans, Samples),
# #                                        use_bias = False,
# #                                        data_format='channels_first')(ch2)
# #     ch3 = Conv2D(12, (1, kernLength), padding = 'same',
# #                                        input_shape = (1, Chans, Samples),
# #                                        use_bias = False,
# #                                        data_format='channels_first')(ch3)
#     print('ch0,ch1,ch2,ch3 after Conv2D shape = {}, {}, {}, {}'.format(ch0.shape, ch1.shape, ch2.shape, ch3.shape))
        
#     block1 = Concatenate(axis=2)([ch0,ch1,ch2,ch3])
#     print('block1 shape after own process = {}'.format(block1.shape))
#     block1       = Activation('elu')(block1)
#     ############################################################################################################
#     # End: Added by GTL to test add layer ####################################################################
#     ############################################################################################################

    
    
    block1       = BatchNormalization(axis = 1)(block1)    # Removed by GTL to remove first Conv2D
